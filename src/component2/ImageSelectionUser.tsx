...
function ImageselectionUser(){
        ...
        Axios.get(`${server_url}/getStatus`).then(res=> {
            if(res.data==="buttonstyle"){
                ...
                if(strrr.length!==0){
                    ...
                    setAnswer(<h1>Ball Went At {res.data} , You Selected {str}</h1>);
    ...
    return(
        <div>
            ...
            <map name="image-map">
                <area id="a1" target="" onMouseOver={(event)=>{change("Deep Extra Cover"); setPhoto(Extra_Cover)}} onClick={(event)=>{handleOnClick(event);changeselect("Deep Extra Cover");}} 
                    alt="Deep Extra Cover" title="Deep Extra Cover" href="" coords="216,151,170,241,183,361,513,254,405,215" shape="poly" />

                <area id="a1" target="" onMouseOver={(event)=>{change("Long Off"); setPhoto(Long_Off)}} onClick={(event)=>{handleOnClick(event);changeselect("Long Off");setPhoto(Long_Off)}} 
                    alt="Long Off" title="Long Off" href="" coords="198,375,267,437,379,505,514,275" shape="poly" />

                <area id="a1" target="" onMouseOver={(event)=>{change("Long On"); setPhoto(Long_On)}} onClick={(event)=>{handleOnClick(event);changeselect("Long On")}} 
                    alt="Long On" title="Long On" href="" coords="407,521,525,560,672,584,547,280" shape="poly" />

                <area target="" onMouseOver={(event)=>{change("Deep MId WIcket"); setPhoto(Deep_Mid_Wicket)}} onClick={(event)=>{handleOnClick(event);changeselect("Deep Mid Wicket")}} 
                    alt="Deep MId WIcket" title="Deep MId WIcket" href="" coords="721,585,841,533,910,413,574,275" shape="poly" />

                <area target="" onMouseOver={(event)=>{change("Deep Backward Square Leg"); setPhoto(Deep_Backward_Square_Leg)}} onClick={(event)=>{handleOnClick(event);changeselect("Deep Backward Square Leg")}} 
                    alt="Deep Backward Square Leg" title="Deep Backward Square Leg" href="" coords="929,374,925,274,854,170,584,257" shape="poly" />
                
                <area target="" onMouseOver={(event)=>{change("FIne Leg"); setPhoto(Fine_Leg)}} onClick={(event)=>{handleOnClick(event);changeselect("Fine Leg")}} 
                    alt="FIne Leg" title="FIne Leg" href="" coords="651,78,717,101,820,152,569,239" shape="poly" />

                <area target="" onMouseOver={(event)=>{change("3rd Man"); setPhoto(third_Man)}} onClick={(event)=>{handleOnClick(event);changeselect("3rd Man")}} 
                    alt="3rd Man" title="3rd Man" href="" coords="447,43,541,60,628,75,540,227" shape="poly" />

                <area target="" onMouseOver={(event)=>{change("Backward Point"); setPhoto(Backward_point)}} onClick={(event)=>{handleOnClick(event);changeselect("Backward Point")}} 
